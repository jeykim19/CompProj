...
class World{
  ...
  int numSpheres;
  int numShips;
  SpaceDS<Sphere> asteroids = new SpaceDS<Sphere>();
  Planet planet;
  MyShip myShip;
  Boolean removingAst = false;
  SpaceDS<Sphere> allDebris= new SpaceDS<Sphere>();
  SpaceDS<Sphere> bullets = new SpaceDS<Sphere>();
  SpaceDS<Sphere> freeShips = new SpaceDS<Sphere>();
  SpaceDS<Sphere> capturedShips = new SpaceDS<Sphere>();
...
// helper method for drawSpheres(), draw entities stored in linked list
  public void drawList(SpaceDS list, Graphics2D g2D){
...
// helper method for updateSphere(), update entities stored in linked list
  public void updateList(SpaceDS list,double time){
    if (list.end!=null) {
      numSpheres = list.length();
      Node<Sphere> iNode = list.end;
      for (int i = 0; i < numSpheres; i ++){
...
public class SpaceAdventure extends JPanel implements KeyListener{
...
//takes in key input and causes change in velocity and production of bullet
    ...
    //takes in key released input and controls behavior
    ...
    public static void main(String[] args){
      ...
      SpaceAdventure mainInstance = new SpaceAdventure();
      ...
      exitButton.addActionListener(new ActionListener(){              // https://www.youtube.com/watch?v=mH1TltI61yU
